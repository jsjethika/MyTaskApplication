...
import com.android.fluktask.mytaskapplication.pojos.upcomingMovie.Results;
...
public class UpcomingMovieGridAdapter extends BaseAdapter {

    private static final String TAG = UpcomingMovieGridAdapter.class.getName();
    ...
    public UpcomingMovieGridAdapter(Activity activity, UpcomingMovieResponse upcomingMovieResponse) {

        this.activity = activity;
        this.upcomingMovieResponse = upcomingMovieResponse;
        mInflater = (LayoutInflater) activity.getSystemService(Context.LAYOUT_INFLATER_SERVICE);


        for(int i=0 ;i < upcomingMovieResponse.getResults().length;i++){
            itemResponseList.add(this.upcomingMovieResponse.getResults()[i]);
        }
